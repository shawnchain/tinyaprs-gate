 ...
 */
void
hash_md5_update(MD5_CTX        *context, unsigned char  *input, unsigned int    inputLen)
{
 ...
 */
void
hash_md5_final(unsigned char   digest[16], MD5_CTX        *context)
{
...
int hash_md5_buffer(char *buffer, unsigned int len, unsigned char digest[16])
{
	...
	hash_md5_init(&context);
	hash_md5_update(&context, (unsigned char *)buffer, len);
	hash_md5_final(digest, &context);
	return 0;
}
...
int hash_md5_file(char *filename,unsigned char digest[16])
{
	...
	if ((file = fopen(filename, "rb")) == NULL)
		...
		{
			hash_md5_update(&context, buffer, len);
		}
		hash_md5_final(digest, &context);
